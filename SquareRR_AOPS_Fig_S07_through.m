...
% In this code, we proposed an algorithm to find an optimal point for switching applications.
%This code regenerates the Fig. S7 of the paper's Supplementary Information.
...
% copy transmission spectrum of first waveguide (through)(column 2) to "Process_Data1"
% copy transmission spectrum of second waveguide (drop)(column 3) to "Process_Data2"
...
%% ===================================================================
%%      ==> first waveguide (through) (Process_Data1) <==
%% ===================================================================
...
%%
%%% finding of Local Maximum and minimum of through spectrum
Max_Through=[]; % Local Maximum of through spectrum
pkss=[]; % peaks of each spectrum in the following loop (not matter)
id_MT=[]; % id of Local Maximum of through spectrum
d=[]; % corresponding features

%%% finding of Local Minimum of through spectrum
Min_Through=[]; % Local minimum of through spectrum
pkss1=[]; % peaks of each spectrum in the following loop (not matter)
id_MT1=[]; % id of Local Minimum of through spectrum
d1=[]; % corresponding features

%%% finding of Local Maximum of the second-order derivative of through transmission <===> minimum of through spectrum
Max_Through2=[]; % Local Maximum of second-order derivative of through transmission <===> minimum of through spectrum
pkss2=[]; % peaks of each spectrum in the following loop (not matter)
id_MT2=[]; % id of Local Maximum of the second-order derivative of through spectrum
d2=[]; % corresponding features

for j=0:n_S-1
    %%% finding of Local Maximum of through spectrum
[pks,locs] = findpeaks(Process_Data1(1+(j*n_WL):n_WL+(j*n_WL),2),Process_Data1(1+(j*n_WL):n_WL+(j*n_WL),1)); % finding Local Maximum of through spectrum and theirs locations
...
Max_Through=[Max_Through;pkss]; % concatenating of peaks of each spectrum to achieve Local Maximum of through spectrum
id_MT=[];
d=[];
...
Min_Through=[Min_Through;pkss1]; % concatenating of peaks of each spectrum to achieve Local Maximum of through spectrum
id_MT1=[];
d1=[];

   %%% finding of Local Maximum of the second-order derivative of through transmission <===> minimum of through spectrum ('MinPeakProminence' used for sharp min/max)
[pks2,locs2] = findpeaks(Process_Data1(1+(j*n_WL):n_WL+(j*n_WL),4),Process_Data1(1+(j*n_WL):n_WL+(j*n_WL),1),'MinPeakProminence',0.01); % finding sharp!! Maximum of second-order derivative of through spectrum (sharp minimum of through spectrum) and theirs locations
...
Max_Through2=[Max_Through2;pkss2]; % concatenating of peaks of each spectrum to achieve Local Maximum of through spectrum
id_MT2=[];
d2=[];
end

%%
%%% waterfall plot of first waveguide (through)
...
chr = '2nd-order derivative of transmission spectrums in through port';
title(chr)
get(gca,'fontname')  % shows you what you are using.
set(gca,'fontname','times','fontweight','bold')  % Set it to times
set(gca,'fontsize',16)
